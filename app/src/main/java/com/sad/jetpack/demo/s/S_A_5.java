...
@ExposedService(url = "test://group/s/a/5")
public class S_A_5 implements IExposedService {
    @Override
    public <T> T action(IPCMessenger messenger) {
        Log.e("sad-jetpack","------------->串行异步任务5");
        SADTaskSchedulerClient.newInstance()
                .execute(new SADTaskRunnable<IDataCarrier>("sa5", new ISADTaskProccessListener<IDataCarrier>() {
                    @Override
                    ...
                    public IDataCarrier doInBackground() throws Exception {
                        ...
                        if (dataCarrier==null){
                            dataCarrier= DataCarrierImpl.newInstanceCreator().data("s_a_5").create();
                        }
                        dataCarrier.creator().data("s_a_5").create();
                        Thread.sleep(5000);
                        return dataCarrier;
                    }
                });
