...
@ExposedService(url = "test://group/c/a/5")
public class C_A_5 implements IExposedService {
    @Override
    public <T> T action(IPCMessenger messenger) {
        Log.e("sad-jetpack","------------->并行异步任务5");
        SADTaskSchedulerClient.newInstance()
                .execute(new SADTaskRunnable<IDataCarrier>("ca5", new ISADTaskProccessListener<IDataCarrier>() {
                    @Override
                    ...
                    public IDataCarrier doInBackground() throws Exception {
                        ...
                        if (dataCarrier==null){
                            dataCarrier= DataCarrierImpl.newInstanceCreator().data("c_a_5").create();
                        }
                        dataCarrier.creator().data("c_a_5").create();
                        Thread.sleep(5000);
                        return dataCarrier;
                    }
                });
