            ...
            if (!TextUtils.isEmpty(url)){
                ...
                IPCComponentProcessorSession processorSession=new IPCComponentProcessorSession() {
                    ...
                    public void onProcessorOutput(String processorId, Message message) {
                        try {
                            if (replyMessenger!=null){
                                IPCMessageTransmissionConfig config=IPCMessageTransmissionConfigImpl.fromCurrProcess()
                                        .destType(CommonConstant.CREATE_REMOTE_IPC_CHAT)
                                        .target(IPCTargetImpl.newInstance()
                                                .toApp(tc.fromApp())
                                                .toProcess(tc.fromProcess())
                                                .url(url)
                                                .delay(target.delay())
                                                .timeout(target.timeout())
                                                .processorMode(target.prcessorMode())
                                        );
                                MessageCreator.standardMessage(message,config);
